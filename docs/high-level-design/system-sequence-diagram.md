...
sequenceDiagram
  ...
  Customer->>API: POST /shipments (Idempotency-Key)
  API->>Ship: validate + create command
  Ship->>DB: tx(write aggregate + outbox)
  DB-->>Ship: committed
  Outbox->>Bus: publish shipment.created.v1
  Bus->>ETA: created event
  ETA-->>Bus: eta.updated.v1
  Bus->>Notif: created + eta events
  Notif-->>Customer: push/email/sms update
```
