...
```mermaid
flowchart TB
  User[Customer / Ops / Driver]
  subgraph Logistics Tracking System
    API[API Container]
    Core[Core Domain Container]
    Track[Tracking Ingestion Container]
    Stream[Event Streaming Container]
    Obs[Observability Container]
  end
  Partners[Carrier/Map/Notification Integrations]
  User --> API
  API --> Core
  API --> Track
  Core --> Stream
