...
```mermaid
flowchart LR
  CreateReq[Create Shipment API] --> Validate[Validation Engine]
  Validate --> Persist[(Shipment DB)]
  Persist --> Outbox[Outbox Table]
  Outbox --> Broker[(Event Bus)]
  Broker --> Milestones[Milestone Processor]
  Broker --> ETA[ETA Recalculator]
  Broker --> Notify[Notification Service]
  Broker --> Audit[(Audit Store)]
  Milestones --> StatusAPI[Tracking Query API]
  ETA --> StatusAPI
  StatusAPI --> Customer[Customer App]
```
