# BPMN Swimlane Diagram (Mermaid Approximation)

```mermaid
flowchart LR
  subgraph Customer
    C1[Submit Shipment Request]
    C2[Receive Tracking Updates]
  end
  subgraph Operations
    O1[Validate & Confirm]
    O2[Assign Pickup Slot]
    O3[Resolve Exceptions]
  end
  subgraph Driver_Hub
    D1[Pickup Scan]
    D2[Hub Arrival/Departure Scans]
    D3[Delivery Attempt + POD]
  end
  subgraph Platform
    P1[Emit Domain Events]
    P2[Recompute ETA]
    P3[Notify Stakeholders]
  end
  C1 --> O1 --> O2 --> D1 --> P1 --> D2 --> P2 --> C2
  D3 --> P1 --> P3 --> C2
  D2 --> O3 --> P1
```
