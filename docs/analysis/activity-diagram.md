...
```mermaid
flowchart TD
  A[Create Shipment] --> B{Validation Passed?}
  B -- No --> X[Reject + Error Event]
  B -- Yes --> C[Confirm Shipment]
  C --> D[Schedule Pickup]
  D --> E[Pickup Scan]
  E --> F[In Transit Scans]
  F --> G{Exception?}
  G -- Yes --> H[Open Exception Workflow]
  H --> I{Resolved?}
  I -- Yes --> F
  I -- No --> J[Terminal Exception State]
  G -- No --> K[Out for Delivery]
  K --> L{Delivered?}
  L -- Yes --> M[Close Shipment]
  L -- No --> H
```
